 ...
 * 
 * @author bfforex
 * @date 2025-10-29 15:09:12 UTC
 * @version 1.3.0
 * @fixed Added defensive checks for path validation
...
function calculateShortCircuit(busId, method = 'point-to-point') {
    ...
    // ✅ DEFENSIVE CHECK: Validate path before proceeding
    if (!path) {
        console.error(`❌ Cannot trace path for bus: ${bus.name}`);
        throw new Error(`Cannot trace path to source for bus "${bus.name}". Ensure bus is connected to a source bus.`);
    }
    
    if (!Array.isArray(path)) {
        console.error(`❌ Invalid path type for bus: ${bus.name}`, typeof path);
        throw new Error(`Path must be an array, got ${typeof path}`);
    }
    
    if (path.length === 0) {
        console.error(`❌ Empty path for bus: ${bus.name}`);
        throw new Error(`Path is empty for bus "${bus.name}". Bus may not be connected to source.`);
    }
    
    console.log(`✅ Path validated: ${path.length} segment(s)`);
...
function calculateShortCircuitPointToPoint(path) {
    ...
    // Added: 2025-10-29 15:09:12 UTC by bfforex
    // Fix for: Cannot read properties of undefined (reading 'length')
    // ═══════════════════════════════════════════════════════════
    
    if (!path) {
        console.error('❌ FATAL: path is null or undefined in calculateShortCircuitPointToPoint');
        throw new Error('Path parameter is required but was not provided');
    }
    
    if (!Array.isArray(path)) {
        console.error('❌ FATAL: path is not an array:', typeof path, path);
        throw new Error(`Path must be an array of segments, got ${typeof path}`);
    }
    
    if (path.length === 0) {
        console.error('❌ FATAL: path array is empty');
        throw new Error('Path array has no segments - bus may not be connected to source');
    }
    
    console.log(`🔵 P-to-P calculation starting: ${path.length} segment(s)`);
    ...
    if (typeof calculateTotalMotorContribution === 'function') {
        motorContribution = calculateTotalMotorContribution(targetBus.id, contributionType || 'interrupting');
    
        // ✅ DEFENSIVE CHECK: Validate motorContribution structure
        // Fixed: 2025-10-29 15:29:14 UTC by bfforex
        if (motorContribution && 
            motorContribution.motors && 
            Array.isArray(motorContribution.motors) && 
            motorContribution.motors.length > 0) {
...
function calculateShortCircuitPerUnit(path) {
    ...
    if (!path) {
        console.error('❌ FATAL: path is null or undefined in calculateShortCircuitPerUnit');
        throw new Error('Path parameter is required but was not provided');
    }
    ...
    console.log(`🔵 Per-Unit calculation starting: ${path.length} segment(s)`);
    ...
    let motorContribution = null;
    if (typeof calculateTotalMotorContribution === 'function') {
        motorContribution = calculateTotalMotorContribution(targetBus.id, 'interrupting');
...
// Updated: 2025-10-29 15:09:12 UTC by bfforex - Added defensive checks
// ═══════════════════════════════════════════════════════════
...
console.log('   - Defensive path validation: ENABLED');
