 ...
 * 
 * @author bfforex
 * @date 2025-10-29 12:13:45 UTC
 * @version 2.0.0 - Enhanced with Demand & Diversity Factors (Feature #5)
 */
...
// Added: 2025-10-29 12:13:45 UTC by bfforex
...
function applyDemandFactorsToLoadFlow(loadFlow) {
    ...
    const enhanced = {
        ...
        demandSummary: {
            connectedCurrent: loadFlow.summary.totalCurrent,
            demandCurrent: 0,
            diversityCurrent: 0,
            connectedPowerKVA: loadFlow.summary.totalPowerKVA,
            demandPowerKVA: 0,
            diversityPowerKVA: 0,
            demandFactor: 1.0,
            diversityFactor: 1.0
    ...
    const diversityFactor = getDiversityFactorForBus(loadFlow.busId) || 0.85;
    ...
    steps += `Connected Load Current:    ${loadFlow.summary.totalCurrent.toFixed(2)} A (100%)\n`;
    steps += `Demand Load Current:       ${enhanced.demandLoad.toFixed(2)} A (${(enhanced.demandLoad / loadFlow.summary.totalCurrent * 100).toFixed(1)}%)\n`;
    steps += `Diversity Load Current:    ${enhanced.diversityLoad.toFixed(2)} A (${(enhanced.diversityLoad / enhanced.demandLoad * 100).toFixed(1)}%)\n`;
    steps += `\n`;
    steps += `Overall Demand Factor:     ${(enhanced.demandLoad / loadFlow.summary.totalCurrent).toFixed(3)}\n`;
    steps += `Overall Diversity Factor:  ${diversityFactor.toFixed(3)}\n`;
    steps += `Combined Factor:           ${(enhanced.diversityLoad / loadFlow.summary.totalCurrent).toFixed(3)}\n`;
    steps += `\n`;
    
    const voltage = loadFlow.busVoltage;
    ...
    enhanced.demandSummary.demandFactor = enhanced.demandLoad / loadFlow.summary.totalCurrent;
    enhanced.demandSummary.diversityFactor = diversityFactor;
    
    steps += `Connected Power:           ${loadFlow.summary.totalPowerKVA.toFixed(2)} kVA\n`;
    ...
    steps += `\n`;
    steps += `Power Savings:             ${(loadFlow.summary.totalPowerKVA - enhanced.demandSummary.diversityPowerKVA).toFixed(2)} kVA\n`;
    steps += `Reduction:                 ${((1 - enhanced.diversityLoad / loadFlow.summary.totalCurrent) * 100).toFixed(1)}%\n`;
    ...
    console.log(`   Connected: ${loadFlow.summary.totalCurrent.toFixed(2)} A → Demand: ${enhanced.demandLoad.toFixed(2)} A → Diversity: ${enhanced.diversityLoad.toFixed(2)} A`);
    console.log(`   Reduction: ${((1 - enhanced.diversityLoad / loadFlow.summary.totalCurrent) * 100).toFixed(1)}%`);
    console.log('');
...
console.log('✅ Load Flow Calculation module loaded with Demand & Diversity Factors');