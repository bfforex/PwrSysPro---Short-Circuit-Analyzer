...
function calculateVoltageDrop(busId, path, loadFlowData = null) {
  ...
  for (let i = startIndex; Array.isArray(path) && i < path.length; i++) {
    ...
    if (comp.type === 'cable') {
      ...
      // Get load current (prefer downstream)
      let loadCurrent = 0;
      try {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ✅ PART 5: FINAL SUMMARY & COMPLIANCE CHECKING
  // This section executes ONCE after ALL components are processed
  // ═══════════════════════════════════════════════════════════════════════
  
  // Calculate final totals (after all components)
  const finalVoltageAtLoad = currentVoltage;
  const totalVoltageDrop = sourceVoltage - finalVoltageAtLoad;
  const totalVoltageDropPercent = (sourceVoltage > 0) ? (totalVoltageDrop / sourceVoltage) * 100 : 0;
  ...
  steps += '-'.repeat(80) + '\n';
  steps += `Source Voltage:          ${sourceVoltage.toFixed(2)} V (100.00%)\n`;
  steps += `Voltage at Load:         ${finalVoltageAtLoad.toFixed(2)} V (${((finalVoltageAtLoad/sourceVoltage)*100).toFixed(2)}%)\n`;
