...
function calculateShortCircuitPointToPoint(path) {
    ...
    // Use calculateTotalMotorContribution (bus-level) and pass a properly-shaped
    // systemFaultResults object into combineSystemAndMotorFault so the motor
    // module's validation succeeds.
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    if (typeof calculateTotalMotorContribution === 'function') {
        // call the bus-level aggregator (calculateTotalMotorContribution) instead of calculateMotorContribution
        ...
        if (motorContribution && motorContribution.motorCount > 0) {
            ...
            // Combine motor impedance with system impedance (use combineSystemAndMotorFault if available)
            if (typeof combineSystemAndMotorFault === 'function') {
                try {
                    // Build a minimal systemFaultResults object with the fields motor module expects
                    const systemFaultStub = {
                        faultCurrents: {
                            // threePhaseSym expected in kA
                    ...
                    if (combined && typeof combined.totalR !== 'undefined') {
                        // Update totals from combined result
                        totalR = combined.totalR;
                        totalX = combined.totalX;
                        totalZ = combined.totalZ;
                        // Append any combined calculation steps (if present)
                        ...
                        // Note: combineSystemAndMotorFault returns fault currents in kA already for threePhaseSym
                    } else {
                        // If combine returned unexpected shape, fall back to manual parallel combine
                ...
                // combineSystemAndMotorFault not available — do manual parallel combine
                const motR = motorContribution.totalMotorR;
                const motX = motorContribution.totalMotorX;
                const sys_R = totalR, sys_X = totalX;
                const sys_Z_sq = sys_R * sys_R + sys_X * sys_X;
    ...
    if (motorContribution && motorContribution.motors && motorContribution.motors.length > 0) {
        // motorContribution may provide totalSymmetricalContribution in kA; fall back to motorFaultCurrent (A) if not present
...
function calculateShortCircuitPerUnit(path) {
    ...
    // (This section should remain identical to your original implementation)
    // ═══════════════════════════════════════════════════════════════════════════
    let stepNumber = 2;
    const processedTransformerConnections = new Set();
    ...
    // Use calculateTotalMotorContribution for bus-level aggregation
    // ═══════════════════════════════════════════════════════════
