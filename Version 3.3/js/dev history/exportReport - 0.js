...
function exportBusReport(busId) {
    ...
    report += `Base Impedance: ${bus.results.baseZ ? bus.results.baseZ.toFixed(6) : 'N/A'} Ω\n`;
    report += `Base Current: ${bus.results.baseCurrent ? bus.results.baseCurrent.toFixed(2) : 'N/A'} A\n\n`;
    
    // Per-Unit Impedances
    if (bus.results.totalRpu !== undefined) {
        report += `PER-UNIT IMPEDANCES:\n`;
        report += `${'-'.repeat(100)}\n`;
        report += `R(pu): ${(bus.results.totalRpu || 0).toFixed(6)} pu\n`;
        report += `X(pu): ${(bus.results.totalXpu || 0).toFixed(6)} pu\n`;
        report += `Z(pu): ${(bus.results.totalZpu || 0).toFixed(6)} pu\n\n`;
    }
        ...
        if (vd.components && vd.components.length > 0) {
            ...
            vd.components.forEach((comp, index) => {
                ...
                if (comp.type === 'cable') {
                    ...
                    tagName = (comp.name || 'N/A').substring(0, 20).padEnd(20);
                    if (comp.location) {
...
function exportAllBusesSummary() {
    ...
    if (busesWithDemand.length > 0) {
        ...
        busesWithDemand.forEach(bus => {
            ...
            totalConnected += summary.connectedCurrent || summary.totalCurrent || 0;
            totalDemand += demandSummary.demandCurrent || 0;
            totalDiversity += demandSummary.diversityCurrent || 0;
        });
        
        const avgDemandFactor = totalConnected > 0 ? totalDemand / totalConnected : 1.0;
        const avgDiversityFactor = totalDemand > 0 ? totalDiversity / totalDemand : 1.0;
        ...
        report += `Average Diversity Factor:   ${avgDiversityFactor.toFixed(3)} (${(avgDiversityFactor).toFixed(1)})\n`;
    ...
    if (systemReport.priorityActions && systemReport.priorityActions.length > 0) {
        ...
        systemReport.priorityActions.forEach((rec, index) => {
            report += `${index + 1}. [${rec.severity || 'UNKNOWN'}] ${rec.name || 'Unnamed'}\n`;
            report += `   Bus: ${rec.busName || 'Unknown'} (${rec.busVoltage || 'N/A'}V)\n`;
    ...
    try {
        ...
        downloadTextFile(report, fileName);
        console.log(`✅ System report exported: ${fileName}`);
    } catch (error) {
        console.error('Error downloading report:', error);
        alert(`❌ Error saving report: ${error.message}`);
    }
