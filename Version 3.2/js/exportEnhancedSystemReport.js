...
Medium Voltage Circuit Breakers (≥1kV):
  • Minimum Rating Required: ${(maxFault.value * 1.25).toFixed(0)} kA sym (25% safety margin)
  • Recommended Rating: ${Math.ceil(maxFault.value * 1.25 / 5) * 5} kA sym (next standard rating)
...
Low Voltage Circuit Breakers (<1kV):
  • Minimum Rating Required: ${(maxFault.value * 1.25).toFixed(0)} kA sym
  • Recommended Rating: ${maxFault.value > 65 ? '100' : maxFault.value > 42 ? '65' : '42'} kA sym
  • Typical Equipment: MCCB/ACB
  • Status: ${maxFault.value <= 42 ? '✓ Standard MCCB adequate' : maxFault.value <= 65 ? '⚠️ High-performance MCCB required' : '❌ ACB or current-limiting required'}
...
`;
    ...
    if (transformers.length > 0) {
        const overloaded = transformers.filter(xfmr => {
            const toBus = buses.find(b => b.id === xfmr.toBus);
            if (toBus?.results?.loadFlow?.summary) {
                const current = toBus.results.loadFlow.summary.totalCurrent || 0;
                const voltage = toBus.voltage;
                const power = (current * voltage * Math.sqrt(3)) / 1000;
                return power > xfmr.rating;
            }
            return false;
        }).length;
...
Section 7 - Temperature Rise:
  ✓ All transformers < 100% rated load
  • Average Loading: ${transformers.length > 0 ? (transformers.reduce((sum, xfmr) => {
      const toBus = buses.find(b => b.id === xfmr.toBus);
      if (toBus?.results?.loadFlow?.summary) {
          const current = toBus.results.loadFlow.summary.totalCurrent || 0;
          const voltage = toBus.voltage;
          const power = (current * voltage * Math.sqrt(3)) / 1000;
          return sum + (power / xfmr.rating * 100);
      }
      return sum;
  }, 0) / transformers.length).toFixed(1) : 0}%
...
${'-'.repeat(100)}
Total Program Cost: $${critical > 0 ? '130K-330K' : '110K-300K'}
