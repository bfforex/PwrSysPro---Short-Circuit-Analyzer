...
function calculateVoltageDrop(busId, path, loadFlowData = null) {
  ...
  // ✅ FIX: Update result structure with BOTH property names for compatibility
  vdData.loadVoltage = finalVoltageAtLoad;
  vdData.actualVoltageAtLoad = finalVoltageAtLoad;  // Alternative name
  
  // ✅ CRITICAL: Provide BOTH property names for backward compatibility
  vdData.totalDropVolts = totalVoltageDrop;
  vdData.cumulativeDropVolts = totalVoltageDrop;  // ← REPORTS EXPECT THIS!
  
  vdData.totalDropPercent = totalVoltageDropPercent;
  vdData.cumulativeDropPercent = totalVoltageDropPercent;  // ← REPORTS EXPECT THIS!
  
  // ✅ LOG: Confirm data is set
  console.log(`✅ Voltage Drop Results:`);
  console.log(`   totalDropPercent: ${totalVoltageDropPercent.toFixed(3)}%`);
  console.log(`   cumulativeDropPercent: ${vdData.cumulativeDropPercent.toFixed(3)}%`);
  console.log(`   totalDropVolts: ${totalVoltageDrop.toFixed(2)}V`);
  console.log(`   cumulativeDropVolts: ${vdData.cumulativeDropVolts.toFixed(2)}V`);
