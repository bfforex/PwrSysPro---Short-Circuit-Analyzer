 ...
 * Orchestrates PE Certification, PEC References, Project Info, labels,
 * and arc-flash / external HV study readiness.
 *
 * @author Engr. B. P. Faraon
 * @date 2026-01-05
 * @version 1.1.0
 * @updated 2026-03-04 - Added arc flash / external HV study compliance logic
 */
...
// Arc-flash applicability used by the in-app IEEE 1584 routine
const REGULATORY_ARC_FLASH_LIMITS = {
  minVoltage: 208,
  maxVoltage: 15000
};

function getCalculatedBusesForCompliance() {
  const allBuses = window.buses || [];
  return allBuses.filter(b => b && b.results && b.results.shortCircuit);
}

function getArcFlashEligibleBuses(calculatedBuses) {
  return calculatedBuses.filter(b => {
    const v = Number(b?.voltage);
    return b?.type !== 'source' && Number.isFinite(v) && v >= REGULATORY_ARC_FLASH_LIMITS.minVoltage && v <= REGULATORY_ARC_FLASH_LIMITS.maxVoltage;
  });
}

function getExternalArcFlashStudyBuses(calculatedBuses) {
  return calculatedBuses.filter(b => {
    const status = b?.results?.arcFlashStatus;
    return status === 'external-study-required' || status === 'not-applicable-source';
  });
}
...
function updateComplianceChecklist() {
  ...
  // 4. Arc Flash Analysis / HV Study Status
  const arcFlashEligibleBuses = calculatedBuses.filter(b => {
    const v = Number(b.voltage);
    return b.type !== 'source' && Number.isFinite(v) && v >= 208 && v <= 15000;
  });

  const arcFlashCompletedBuses = arcFlashEligibleBuses.filter(b => b.results?.arcFlash);
  const arcFlashExternalStudyBuses = calculatedBuses.filter(b =>
    b.results?.arcFlashStatus === 'external-study-required' ||
    b.results?.arcFlashStatus === 'not-applicable-source'
  );

  let arcFlashStatus = 'pending';
  let arcFlashCompleteness = 0;
  let arcFlashDetails = '';

  if (arcFlashEligibleBuses.length === 0) {
    arcFlashStatus = 'ready';
    arcFlashCompleteness = 100;
    arcFlashDetails =
      'No in-app eligible buses for arc flash. ' +
      arcFlashExternalStudyBuses.length +
      ' bus(es) marked for external HV/source handling.';
  } else if (arcFlashCompletedBuses.length === arcFlashEligibleBuses.length) {
    arcFlashStatus = 'complete';
    arcFlashCompleteness = 100;
    arcFlashDetails =
      arcFlashCompletedBuses.length +
      ' of ' +
      arcFlashEligibleBuses.length +
      ' eligible buses completed; ' +
      arcFlashExternalStudyBuses.length +
      ' bus(es) external/not-applicable.';
  } else {
    arcFlashStatus = 'review';
    arcFlashCompleteness = (
      arcFlashCompletedBuses.length / arcFlashEligibleBuses.length * 100
    ).toFixed(0);
    arcFlashDetails =
      arcFlashCompletedBuses.length +
      ' of ' +
  ...
  checklist.push({
    category: 'Safety',
    item: 'Arc Flash Analysis / HV Study Status',
    status: arcFlashStatus,
    completeness: arcFlashCompleteness,
    details: arcFlashDetails,
    priority: 'high'
  });
  ...
  try {
    ...
    labelsGenerated = window.faultCurrentLabelsGenerated === true;
  }

  checklist.push({
    category: 'Documentation',
    item: 'Fault Current Labels (NEC 110.24/PEC 1.10.9)',
    status: calculatedBuses.length === 0
      ? 'pending'
      : (labelsGenerated ? 'complete' : 'ready'),
    completeness: calculatedBuses.length === 0
      ? 0
      : 100,
    details: calculatedBuses.length === 0
      ? 'No calculated buses available for labels'
      : (labelsGenerated
          ? 'Fault current labels generated for ' + calculatedBuses.length + ' buses'
          : 'Ready to generate for ' + calculatedBuses.length + ' buses'),
    priority: 'medium'
  });

  // 6. PEC Compliance References
  checklist.push({
    category: 'Standards',
...
function exportPermitSubmissionPackage() {
  ...
  html += '<li>Arc Flash Hazard Analysis / External HV Arc-Flash Study, as applicable</li>';
...
function validateRegulatoryData() {
  ...
  // Check Calculations
  const buses = window.buses || [];
  const calculatedBuses = getCalculatedBusesForCompliance();
  ...
  // Check Arc Flash Analysis / External HV Study status
  const arcFlashEligibleBuses = getArcFlashEligibleBuses(calculatedBuses);
  const missingArcFlash = arcFlashEligibleBuses.filter(b => !b.results?.arcFlash);
  const externalStudyBuses = calculatedBuses.filter(b => b.results?.arcFlashStatus === 'external-study-required');

  if (missingArcFlash.length > 0) {
    validation.warnings.push(missingArcFlash.length + ' eligible bus(es) do not yet have in-app arc flash analysis');
  }

  if (externalStudyBuses.length > 0) {
    validation.warnings.push(externalStudyBuses.length + ' high-voltage bus(es) require external HV arc-flash study');
  }
...
console.log(' - Arc flash / external HV study compliance logic: ENABLED');
